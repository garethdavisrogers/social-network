...
namespace SocialNetworkV1.Services
{
    public interface IAuthService
    {
        ...
        Task<(bool success, string? token, IEnumerable<string> errors)> LoginUserAsync(string userNameOrEmail, string password);
    ...
    public class AuthService : IAuthService
    {
        ...
        public async Task<(bool success, string? token, IEnumerable<string> errors)> LoginUserAsync(string userNameOrEmail, string password)
        {
            var user =  await _userManager.FindByEmailAsync(userNameOrEmail) ?? await _userManager.FindByNameAsync(userNameOrEmail);

            if (user == null) return (false, null, new[] { "Invalid UserName or Email" });
